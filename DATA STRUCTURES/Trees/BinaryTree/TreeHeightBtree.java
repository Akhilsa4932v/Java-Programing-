...
//Diameter.
    public static int Diameter(Node root){ //O(N^2)
        ...
        //DIAMETER OF TREE IN O(N) TC;

//CREATE A INFOMATION CLASS , IT STORES INFO OF DIA AND HEIGHT.
   class Info{
    int diameter;
    int height;

    Info(int dia,int heig){
        this.diameter = dia;
        this.height = heig;
    }
   }
   ...
   Info diameter(Node root){
    ...
    int heig = Math.max(linfo.height, rinfo.height)+1;
    int dia = Math.max(Math.max(linfo.diameter, rinfo.diameter),linfo.diameter+rinfo.diameter+1);

    return new Info(dia,heig);
